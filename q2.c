...
SDL_Window*w;SDL_GLContext c;I prg,vao,vbo,nv,tex;F*vb;V cam;F ya,pi;U pal[768];
...
void BS(const char*p){I sz;U*d=LF(p,&sz);if(!d||sz<8||memcmp(d,"IBSP",4)||((I*)d)[1]!=38){F t[]={-100,-100,0,0,0,0.5,0.5,0,1,0,100,-100,0,1,0,0.5,0.5,0,1,0,0,100,0,0.5,1,0.5,0.5,0,1,0};vb=malloc(sizeof(t));memcpy(vb,t,sizeof(t));nv=3;return;}L*lp=(L*)(d+8);V*vx=(V*)(d+lp[2].o);TI*ti=(TI*)(d+lp[5].o);E*ed=(E*)(d+lp[11].o);J*se=(J*)(d+lp[12].o);FC*fc=(FC*)(d+lp[6].o);I nf=lp[6].l/sizeof(FC);vb=malloc(nf*100*10*sizeof(F));F*vp=vb;F mx=-9e9,my=-9e9,mz=-9e9,Mx=9e9,My=9e9,Mz=9e9;for(I i=0;i<nf;i++){FC*f=&fc[i];TI*tx=&ti[f->ti];V vl[64];I nv_=0;for(I j=0;j<f->ne&&j<64;j++){J si=se[f->fe+j];S vi=si>=0?ed[si].v[0]:ed[-si].v[1];vl[nv_++]=vx[vi];}for(I t=0;t<nv_-2;t++){for(I k=0;k<3;k++){V*v=k==0?&vl[0]:k==1?&vl[t+1]:&vl[t+2];F u=(v->x*tx->v[0][0]+v->y*tx->v[0][1]+v->z*tx->v[0][2]+tx->v[0][3])/256.0f;F vv=(v->x*tx->v[1][0]+v->y*tx->v[1][1]+v->z*tx->v[1][2]+tx->v[1][3])/512.0f;mx=v->x>mx?v->x:mx;my=v->y>my?v->y:my;mz=v->z>mz?v->z:mz;Mx=v->x<Mx?v->x:Mx;My=v->y<My?v->y:My;Mz=v->z<Mz?v->z:Mz;*vp++=v->x;*vp++=v->y;*vp++=v->z;*vp++=u;*vp++=vv;*vp++=0.5;*vp++=0.5;*vp++=1;*vp++=1;*vp++=1;}}}nv=(vp-vb)/10;printf("BSP: %d faces -> %d tris, bounds:[%.0f,%.0f] [%.0f,%.0f] [%.0f,%.0f]\n",nf,nv/3,Mx,mx,My,my,Mz,mz);free(d);}
