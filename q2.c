...
I txm[256];I txc=0;void BS(const char*p){I sz;U*d=LF(p,&sz);if(!d||sz<8||memcmp(d,"IBSP",4)||((I*)d)[1]!=38){F t[]={-100,-100,0,0,0,0.5,0.5,0,1,0,100,-100,0,1,0,0.5,0.5,0,1,0,0,100,0,0.5,1,0.5,0.5,0,1,0};vb=malloc(sizeof(t));memcpy(vb,t,sizeof(t));nv=3;return;}L*lp=(L*)(d+8);V*vx=(V*)(d+lp[2].o);TI*ti=(TI*)(d+lp[5].o);E*ed=(E*)(d+lp[11].o);J*se=(J*)(d+lp[12].o);FC*fc=(FC*)(d+lp[6].o);U*lm=(U*)(d+lp[7].o);I nf=lp[6].l/sizeof(FC);I nt=lp[5].l/sizeof(TI);for(I i=0;i<nt&&txc<256;i++){char pth[128];sprintf(pth,"baseq2/textures/%s.wal",ti[i].n);I tx=LW(pth);txm[i]=tx?tx:tex;}U*la=malloc(512*512*3);memset(la,128,512*512*3);I lx=0,ly=0,lh=0;vb=malloc(nf*100*10*sizeof(F));F*vp=vb;F mx=-9e9,my=-9e9,mz=-9e9,Mx=9e9,My=9e9,Mz=9e9;for(I i=0;i<nf;i++){FC*f=&fc[i];TI*tx=&ti[f->ti];V vl[64];I nv_=0;F mu=9e9,mv=9e9,Mu=-9e9,Mv=-9e9;for(I j=0;j<f->ne&&j<64;j++){J si=se[f->fe+j];S vi=si>=0?ed[si].v[0]:ed[-si].v[1];vl[nv_]=vx[vi];F u=vl[nv_].x*tx->v[0][0]+vl[nv_].y*tx->v[0][1]+vl[nv_].z*tx->v[0][2]+tx->v[0][3];F vv=vl[nv_].x*tx->v[1][0]+vl[nv_].y*tx->v[1][1]+vl[nv_].z*tx->v[1][2]+tx->v[1][3];mu=u<mu?u:mu;mv=vv<mv?vv:mv;Mu=u>Mu?u:Mu;Mv=vv>Mv?vv:Mv;nv_++;}I lw=(I)((Mu-mu)/16)+1,lh_=(I)((Mv-mv)/16)+1;if(lx+lw>512){lx=0;ly+=lh+1;lh=0;}if(f->lm>=0&&f->lm<lp[7].l){for(I y=0;y<lh_;y++)for(I x=0;x<lw;x++){I si=(lx+x+(ly+y)*512)*3,di=f->lm+(x+y*lw)*3;if(di+2<lp[7].l){la[si]=lm[di];la[si+1]=lm[di+1];la[si+2]=lm[di+2];}}}for(I t=0;t<nv_-2;t++){for(I k=0;k<3;k++){V*v=k==0?&vl[0]:k==1?&vl[t+1]:&vl[t+2];F u=(v->x*tx->v[0][0]+v->y*tx->v[0][1]+v->z*tx->v[0][2]+tx->v[0][3])/256.0f;F vv=(v->x*tx->v[1][0]+v->y*tx->v[1][1]+v->z*tx->v[1][2]+tx->v[1][3])/512.0f;F lu=(v->x*tx->v[0][0]+v->y*tx->v[0][1]+v->z*tx->v[0][2]+tx->v[0][3]-mu)/16.0f;F lv=(v->x*tx->v[1][0]+v->y*tx->v[1][1]+v->z*tx->v[1][2]+tx->v[1][3]-mv)/16.0f;mx=v->x>mx?v->x:mx;my=v->y>my?v->y:my;mz=v->z>mz?v->z:mz;Mx=v->x<Mx?v->x:Mx;My=v->y<My?v->y:My;Mz=v->z<Mz?v->z:Mz;*vp++=v->x;*vp++=v->y;*vp++=v->z;*vp++=u;*vp++=vv;*vp++=(lx+lu)/512.0f;*vp++=(ly+lv)/512.0f;*vp++=f->ti/255.0f;*vp++=1;*vp++=1;}}lx+=lw+1;lh=lh_>lh?lh_:lh;}nv=(vp-vb)/10;glGenTextures(1,&lmt);glBindTexture(GL_TEXTURE_2D,lmt);glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,512,512,0,GL_RGB,GL_UNSIGNED_BYTE,la);glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);free(la);printf("BSP: %d faces -> %d tris, %d textures, bounds:[%.0f,%.0f] [%.0f,%.0f] [%.0f,%.0f]\n",nf,nv/3,nt,Mx,mx,My,my,Mz,mz);free(d);}
