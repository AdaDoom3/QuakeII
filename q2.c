...
#define W 1280
#define H 720
typedef unsigned char U;typedef unsigned short S;typedef unsigned int I;typedef int J;typedef float F;
typedef struct{F x,y,z;}V;typedef struct{F m[16];}M;typedef struct{I o,l;}L;typedef struct{S v[2];}E;
typedef struct{J e;U d;}FE;typedef struct{S p,n,fe,ne,ti;U s[4];J lm,lo[2];U lw,lh,ls[2];}FC;
...
U*LB(const char*p,I*sz){FILE*f=fopen(p,"rb");if(!f)return 0;fseek(f,0,2);*sz=ftell(f);fseek(f,0,0);U*b=malloc(*sz);fread(b,1,*sz,f);fclose(f);return b;}
...
M PR(F fov,F asp,F n,F f){M r=ID();F t=1.0f/tanf(fov*0.5f);r.m[0]=t/asp;r.m[5]=t;r.m[10]=-(f+n)/(f-n);r.m[11]=-1;r.m[14]=-(2*f*n)/(f-n);r.m[15]=0;return r;}
...
void BS(const char*p){I sz;U*d=LB(p,&sz);if(!d||sz<8||memcmp(d,"IBSP",4)||((I*)d)[1]!=38){MK();return;}L*lp=(L*)(d+8);V*vx=(V*)(d+lp[2].o);E*ed=(E*)(d+lp[11].o);FE*fe=(FE*)(d+lp[12].o);FC*fc=(FC*)(d+lp[6].o);I nv=lp[2].l/12,ne=lp[11].l/4,nfe=lp[12].l/4,nfc=lp[6].l/sizeof(FC);printf("BSP: %d verts %d faces\n",nv,nfc);I nt=0;for(I i=0;i<nfc;i++)if(fc[i].ne>=3&&fc[i].fe>=0&&fc[i].fe<nfe)nt+=(fc[i].ne-2)*3;vbuf=malloc(nt*10*sizeof(F));F*vp=vbuf;for(I i=0;i<nfc;i++){FC*f=&fc[i];if(f->ne<3||f->fe<0||f->fe>=nfe)continue;for(I j=1;j<f->ne-1;j++){I idx[3]={0,j,j+1};for(I k=0;k<3;k++){I fi=f->fe+idx[k];if(fi>=nfe)continue;I ei=fe[fi].e;if(abs(ei)>=ne)continue;I ev=ei<0?1:0;V*v=&vx[ed[abs(ei)].v[ev]];*vp++=v->x;*vp++=v->y;*vp++=v->z;*vp++=0;*vp++=0;*vp++=0.5;*vp++=0.5;*vp++=0;*vp++=0;*vp++=1;}}}nverts=(vp-vbuf)/10;free(d);printf("Tris: %d\n",nverts/3);}
...
int main(int argc,char**argv){cam=(V){0,0,300};IN();SH();BS(argc>1?argv[1]:"baseq2/maps/sample.bsp");DR();for(int i=0;i<60;i++){RN();SDL_Delay(16);}SS("phase2.ppm");SDL_Quit();return 0;}
